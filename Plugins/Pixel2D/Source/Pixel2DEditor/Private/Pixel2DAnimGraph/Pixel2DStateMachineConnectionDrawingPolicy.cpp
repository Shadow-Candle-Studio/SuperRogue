...
void FPixel2DStateMachineConnectionDrawingPolicy::DetermineWiringStyle(UEdGraphPin* OutputPin, UEdGraphPin* InputPin, /*inout*/ FConnectionParams& Params)
{
	...
	if (InputPin)
	{
		if (UPixel2DAnimStateTransitionNode* TransNode = Cast<UPixel2DAnimStateTransitionNode>(InputPin->GetOwningNode()))
		{
...
{
	if (UPixel2DAnimStateEntryNode* EntryNode = Cast<UPixel2DAnimStateEntryNode>(OutputPin->GetOwningNode()))
	{
		StartWidgetGeometry = PinGeometries->Find(OutputPinWidget);

		UPixel2DAnimStateNodeBase* State = CastChecked<UPixel2DAnimStateNodeBase>(InputPin->GetOwningNode());
		int32 StateIndex = NodeWidgetMap.FindChecked(State);
		EndWidgetGeometry = &(ArrangedNodes[StateIndex]);
	}
	else if (UPixel2DAnimStateTransitionNode* TransNode = Cast<UPixel2DAnimStateTransitionNode>(InputPin->GetOwningNode()))
	{
