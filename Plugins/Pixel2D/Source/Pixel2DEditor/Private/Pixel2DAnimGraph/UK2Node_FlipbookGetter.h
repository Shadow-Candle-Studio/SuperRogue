...
#include "UK2Node_FlipbookGetter.generated.h"
...
USTRUCT()
struct FPixel2DNodeSpawnData
{
	GENERATED_BODY()

	FPixel2DNodeSpawnData();
	...
	UPROPERTY()
	UPixel2DAnimGraphNode_Base* SourceNode;

	// The state node the spawned getter accesses
	UPROPERTY()
	UPixel2DAnimStateNodeBase* SourceStateNode;
	...
	UPROPERTY()
	const UPixel2DAnimBlueprint* SourceBlueprint;
...
UCLASS(MinimalAPI)
class UK2Node_FlipbookGetter : public UK2Node_CallFunction
{
	GENERATED_BODY()
public:
	...
	// UPixel2DAnimStateNode doesn't use the same hierarchy so we need to have a seperate property here to handle
	// those.
	UPROPERTY()
	UPixel2DAnimStateNodeBase* SourceStateNode;
	...
	UPROPERTY()
	const UPixel2DAnimBlueprint* SourceAnimBlueprint;
...
protected:
	...
	void PostSpawnNodeSetup(UEdGraphNode* NewNode, bool bIsTemplateNode, FPixel2DNodeSpawnData SpawnData);
};
